...
if [ $# != 3 ]; then
  echo "Usage: $0 <audio-path> <text-path> <data-path>"
  echo " $0 /export/corpora/LDC03S04 /export/corpora/LDC03T19 ./data"
  exit 1;
fi
...
python2 -c "import mmseg" 2>/dev/null || {
    echo "Python module mmseg is not found. Please install mmseg in python2."; exit 1; }
...
cat $train_dir/transcripts.txt |\
  ...
  sed -e 's/((\([^)]\{0,\}\)))/\1/g' |\
  local/HKUST/data_prep/hkust_normalize.pl |\
  local/HKUST/data_prep/hkust_segment.py |\
