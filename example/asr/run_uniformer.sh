...
# model 1 : MTA transformer

# The training related config
task_file=eteh.lib.std_asr_task
task_name=CtcAttTask_Kaldi
train_config_template=conf/e2e/eteh_baseline_uniformer_mta.yaml
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    echo "Model average"
	decode_model=model.attaccbest10avg
    best10=`python3 local/model_average/best_epochs.py ${exp_dir}/rank0_train.log att_corr_off 10 True`
...
# model 2 : TMTA transformer

exp_dir_mta=$exp_dir

# The training related config
train_config_template=conf/e2e/eteh_baseline_uniformer_tmta.yaml
exp_dir=exp/$train_job_name/$(basename ${train_config_template%.*})
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
    ...
    python ${PLAT_ROOT}/bin/train_dist.py -train_config ${train_config} \
        ...
        -gpus ${node_gpu} \
        -node_id ${node_idx} \
        -checkpoint $exp_dir_mta/checkpoint.39 \
        --resume_optimizer \
        --resume_progress
