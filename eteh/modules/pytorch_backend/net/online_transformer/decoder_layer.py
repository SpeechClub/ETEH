...
class DecoderLayer(nn.Module):
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask, cache=None, ep=None):
        ...
        if self.concat_after:
            if ep is None:
                ...
                x_concat = torch.cat(
                    (x, self.src_attn(x, memory, memory, memory_mask, ep)), dim=-1
                )
            ...
            if ep is None:
                ...
                x = residual + self.dropout(self.src_attn(x, memory, memory, memory_mask, ep))
        if not self.normalize_before:
            x = self.norm2(x)
