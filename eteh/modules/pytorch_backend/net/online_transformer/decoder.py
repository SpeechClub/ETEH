...
from eteh.modules.pytorch_backend.net.transformer.attention import MultiHeadedAttention
from eteh.modules.pytorch_backend.net.transformer.embedding import PositionalEncoding
from eteh.modules.pytorch_backend.net.transformer.layer_norm import LayerNorm
from eteh.modules.pytorch_backend.net.transformer.positionwise_feed_forward import PositionwiseFeedForward
from eteh.modules.pytorch_backend.net.transformer.repeat import repeat
...
class StreamDecoder(torch.nn.Module):
    ...
    def __init__(self, odim,
        ...
        self.decoders = repeat(
            ...
            lambda lnum: DecoderLayer(
                attention_dim,
                MultiHeadedAttention(self_attention_heads, attention_dim, self_attention_dropout_rate),
                MTMultiHeadedAttention(src_attention_heads, attention_dim, src_attention_dropout_rate, \
                                       src_attention_bias_init, src_attention_sigmoid_noise),
    ...
    def forward_one_step_online(self, tgt, tgt_mask, memory, cache=None):
        """Forward one step.
        ...
        for c, decoder in zip(cache, self.decoders):
            x, tgt_mask, memory, memory_mask, = decoder(x, tgt_mask, memory, None, cache=c[0], ep=c[1])
    ...
    def forward_one_step_offline(self, tgt, tgt_mask, memory, cache=None):
        """Forward one step.
        :param torch.Tensor tgt: input token ids, int64 (batch, maxlen_out)
        :param torch.Tensor tgt_mask: input token mask,  (batch, maxlen_out)
                                      dtype=torch.uint8 in PyTorch 1.2-
    ...
    def forward_one_step(self, tgt, tgt_mask, memory, cache=None, online=False):
        if online:
            return self.forward_one_step_online(tgt, tgt_mask, memory, cache=cache)
        else:
            return self.forward_one_step_offline(tgt, tgt_mask, memory, cache=cache)
